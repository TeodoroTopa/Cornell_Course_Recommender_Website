...
I completed this full-stack development website project on a team of 5 people for the course INFO 4300: Language and Information, in the Spring of 2021. The final product is a website (no longer live :( ) that delivers course recommendations to users who input a natural language query (along with course parameter filters). The suggested courses were ranked based on similarity to the query, and compared with one another using tf-idf values and cosine similarity. The website was integrated with and displayed ratemyprofessor.com rankings, and used fuzzy matching to match course teacher names to the Cornell course roster. The website was also equipped with a user login and a course favoriting system. Once a user favorited a class, a Rocchio algorithm was implemented to update future queries to better reflect the user's expressed course preferences. The website (BigRedCourseRecommender) earned a spot in the class's 'Hall of Fame', and can be found at https://4300-hall-of-fame.infosci.cornell.edu/. On the team, I was primarily responsible for front-end development, ratemyprofessor integration, and tfidf/cosine similarity precomputing.

Everything from this point down is instructions from the original repo that my team forked when we started the project.
------------------------------------------------------
