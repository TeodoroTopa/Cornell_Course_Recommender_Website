...
net_id = "js3548, yz366, mba93, tst42, mkc236"
...
input_to_data = {
    ...
    "other-location": (["locationDescr"], ["Geneva, NY", "AAP in NYC", "Beijing, China",
                                           "Engineering in NYC", "Human Ecology in NYC", "Other Domestic",
                                           "Other International",
...
def remove_cross_listings(rankings):
    ...
    for index in rankings:
        ...
        if (course_ourId in ourId_to_titleLong.keys()):
            ...
            cross_list_string = " Cross-listed with " + str(course_contents[index]['subject']) + " " + str(
                course_contents[index]['catalogNbr'] + ".")
            if (each_course_info[idx]['description'] != None and each_course_info[idx]['description'].find(
                    cross_list_string) == -1):
...
def get_filters():
    global class_levels
    class_levels_response = [];
    class_level_boolean = True
    ...
    grading_scheme_response = [];
    grading_scheme_boolean = True
...
def saved():
    if google_auth.is_logged_in():
        ...
        if (result == []):
            ...
            return render_template('saved.html', is_logged=True, data=data,
                                   username=google_auth.get_user_info()['given_name'])
